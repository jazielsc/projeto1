...
class Class_item2
{
		...
		// se retornou então envia a mensagem
            if ($quant > 0)
            {
            ...
            } else
                 {
                    ...
                    $query_grid = mysql_query("SELECT cod_item2, aluno.nome, disciplina.nome, curso.nome FROM item2, disciplina, aluno, curso WHERE curso.cod_curso = disciplina.cod_curso AND item2.cod_disciplina = disciplina.cod_disciplina AND item2.cod_aluno = aluno.cod_aluno AND item2.cod_disciplina = '$cod_disciplina' AND aluno.cod_status = 1 ORDER BY aluno.nome",$conn) or die ("Error na consulta");
     ...
     public function select_grid_item2_disciplina($cod_aluno,$ano) { // método para preenchimento da grid no form
         ...
         $query_grid = mysql_query("SELECT cod_item2, aluno.nome, disciplina.nome, curso.nome, turma.nome FROM item2, disciplina, aluno, curso, turma WHERE turma.cod_turma = disciplina.cod_turma AND turma.cod_curso = curso.cod_curso AND curso.cod_curso = disciplina.cod_curso AND item2.cod_disciplina = disciplina.cod_disciplina AND item2.cod_aluno = aluno.cod_aluno AND aluno.cod_aluno = '$cod_aluno' AND aluno.cod_status = 1 $where ORDER BY aluno.nome",$conn) or die (mysql_error(). " Error na consulta");
	...
	if ($quant > 0)
	{
     ...
     public function update_item2($codigo,$cod_disciplina)
     {
                  ...
                  include($_SERVER['DOCUMENT_ROOT']."/boletim/scripts/conecta.php");
                    ...
                    $query_grid = mysql_query("SELECT cod_item2, aluno.nome, disciplina.nome FROM item2, disciplina, aluno WHERE item2.cod_disciplina = disciplina.cod_disciplina AND item2.cod_aluno = aluno.cod_aluno AND item2.cod_disciplina = '$cod_disciplina' AND aluno.cod_status = 1 ORDER BY aluno.nome",$conn) or die ("Error na consulta");
