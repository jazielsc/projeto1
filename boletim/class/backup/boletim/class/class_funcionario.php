...
class Class_funcionario
{
     ...
     public function insert_funcionario($nome,$email,$cod_cidade,$cod_bairro,$cod_rua,$complemento,$numero,$cod_uf,$telefone,$cod_status,$cod_instituicao,$cep,$cargo,$cod_cargo)
     {
	 ...
	 $quant = mysql_num_rows($query_select);
		...
		// se retornou então envia a mensagem
            ...
            // verificando se a query retornou algum valor
                 ...
                 {
                     $query_insert = mysql_query("INSERT INTO funcionario VALUE (NULL, '$nome','$email','$cod_cidade','$cod_bairro','$cod_rua','$complemento','$numero','$cod_uf','$telefone','$cod_status', '$cod_instituicao','$cep','$cargo')") or die ("Erro insert ". mysql_erro());
                     
                     $query_grid = mysql_query("SELECT cod_funcionario, funcionario.nome, cidade_nome , telefone FROM funcionario, cidade WHERE funcionario.cod_cidade = cidade_id AND cod_instituicao = '$cod_instituicao' ORDER BY funcionario.nome",$conn) or die ("Error na consulta ". mysql_error());
     ...
     public function select_grid_funcionario() { // método para preenchimento da grid no form
         ...
         include($_SERVER['DOCUMENT_ROOT']."/boletim/scripts/conecta.php");
                    ...
                    $query_grid = mysql_query("SELECT cod_funcionario, funcionario.nome, cidade_nome , telefone FROM funcionario, cidade WHERE funcionario.cod_cidade = cidade_id AND cod_instiuicao = '$cod_instituicao' ORDER BY funcionario.nome",$conn) or die ("Error na consulta");
	...
	{	
     ...
     public function update_funcionario($nome,$email,$cod_cidade,$cod_bairro,$cod_rua,$complemento,$numero,$cod_uf,$telefone,$cod_status,$cod_funcionario,$cod_instituicao,$cep,$cargo,$cod_cargo)
     {
         include($_SERVER['DOCUMENT_ROOT']."/boletim/scripts/conecta.php");

         
                     $query_insert = mysql_query("UPDATE funcionario SET nome = '$nome', email = '$email', cod_cidade = '$cod_cidade', cod_bairro = '$cod_bairro', cod_rua = '$cod_rua', complemento = '$complemento', numero = '$numero', cod_uf = '$cod_uf', telefone = '$telefone', cod_status = '$cod_status', cod_instituicao = '$cod_instituicao', cep = '$cep', cargo = '$cargo' WHERE cod_funcionario = '$cod_funcionario'") or die ("Erro insert ". mysql_errno());
             ...
             $query_update = mysql_query("UPDATE usuario SET usuario_atrib = 6 WHERE cod_aluno_professor = '$cod_funcionario' AND referencia = 3") or die("ERRO!");
              ...
              $query_update = mysql_query("UPDATE usuario SET usuario_atrib = '$cod_cargo' WHERE cod_aluno_professor = '$cod_funcionario' AND referencia = 3") or die("ERRO!");
                     ...
                     $query_grid = mysql_query("SELECT cod_funcionario, funcionario.nome, cidade_nome , telefone FROM funcionario, cidade WHERE funcionario.cod_cidade = cidade_id ORDER BY funcionario.nome",$conn) or die ("Error na consulta");
