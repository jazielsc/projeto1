...
class Class_aluno
{
     ...
     public function insert_aluno($nome,$email,$cod_cidade,$cod_bairro,$cod_rua,$complemento,$numero,$cod_uf,$telefone,$cod_status,$cod_instituicao,$matricula,$cep,$responsavel,$email_responsavel,$cod_curso,$identidade,$datanasc)
     {
        session_start();
        $instituicao = $_SESSION['instituicao'];
         ...
         $query_select = mysql_query("SELECT cod_aluno FROM aluno WHERE aluno.email = '$email' OR email_responsavel = '$email_responsavel'") or die ("Erro select ". mysql_error());
	 ...
	 $quant = mysql_num_rows($query_select);
		...
		// se retornou então envia a mensagem
            if ($quant > 0 or $quant2 > 0)
            {
                 ...
                 {
                     $query_insert = mysql_query("INSERT INTO aluno VALUE (NULL, '$nome','$email','$cod_cidade','$cod_bairro','$cod_rua','$complemento','$numero','$cod_uf','$telefone','$cod_status', '$cod_instituicao','$matricula', '$cep','$responsavel','$email_responsavel', '$cod_curso','$identidade','$datanasc')") or die ("Erro insert 0 ". mysql_errno());
                     
                     $query_grid = mysql_query("SELECT cod_aluno, aluno.nome, cidade_nome , telefone FROM aluno, cidade WHERE aluno.cod_cidade = cidade_id AND cod_instituicao = '$cod_instituicao' ORDER BY aluno.nome",$conn) or die ("Error na consulta");
	...
	{	
     ...
     public function update_aluno($nome,$email,$cod_cidade,$cod_bairro,$cod_rua,$complemento,$numero,$cod_uf,$telefone,$cod_status,$cod_aluno,$cod_instituicao,$matricula,$cep,$responsavel,$email_responsavel,$cod_curso,$identidade,$datanasc)
     {
         include($_SERVER['DOCUMENT_ROOT']."/boletim/scripts/conecta.php");
