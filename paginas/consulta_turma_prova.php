<?php 
	...
	if(!isset($_GET['ordem'])){
		...
		switch($_GET['ordem']){
			...
			case 5: $ordem = 'professor.nome'; break;
		}
	}
	
	/* CDIGO PARA O CARREGAMENTO DOS CAMPOS CASO SEJA UMA ALTERAO */
	if(isset($_GET['id'])){
		$recupera = mysql_query("SELECT turma.cod_turma, turma.nome, curso.nome as cnome, turma.turno, turma.semestre, turma.data, turma.data_final FROM turma, curso WHERE turma.cod_instituicao = ".$_SESSION['id_instituicao']." AND turma.cod_curso = curso.cod_curso AND turma.cod_turma = ".$_GET['id']) or die ("Error na consulta");
		...
		<div class="corpo">
				...
				<div style="position: relative; height: 200px; width: 600px; left: 50%; margin-left: -300px; overflow-x: none; overflow-y: scroll; border: 1px solid blue;">
					<center>
						<table>
							...
							<tr>
								...
								<td>
									<select name="consulta" class="campo" style="width: 200px;">
										...
										<?php 
											$query = mysql_query("SELECT cod_turma, nome FROM turma WHERE cod_instituicao = ".$_SESSION['id_instituicao']) or die ("Error na consulta");
							...
							<tr>
								<td class="rotulo">Data de Incio: </td>
								<td><input name="data_inicio" disabled="disabled" value="<?php echo $result[5];?>" class="campo" style="width: 400px;"/></td>
							</tr>
							<tr>
								<td class="rotulo">Data de Termino: </td>
								<td><input name="data_termino" disabled="disabled" value="<?php echo $result[6];?>" class="campo" style="width: 400px;"/></td>
							</tr>
						</table>
					</center>
			</div>
			...
			<div class="lista_selecao" style="width: 730px; height:400px; top: 350px; left: 50%; margin-left: -365px;">
				<table id="box-table-a">
					<thead>
						<tr>
							<th scope="col" style="width: 60px;"><a href="consulta_turma_horario.php?ordem=0&id=<?php echo $_GET['id'];?>">Dia</a></th>
							<th scope="col" style="width: 20px;"><a href="consulta_turma_horario.php?ordem=1&id=<?php echo $_GET['id'];?>">H</a></th>
							<th scope="col" style="width: 100px;"><a href="consulta_turma_horario.php?ordem=2&id=<?php echo $_GET['id'];?>">Incio</a></th>
							<th scope="col" style="width: 100px;"><a href="consulta_turma_horario.php?ordem=3&id=<?php echo $_GET['id'];?>">Termino</a></th>
							<th scope="col" style="width: 150px;"><a href="consulta_turma_horario.php?ordem=4&id=<?php echo $_GET['id'];?>">Disciplina</a></th>
							<th scope="col" style="width: 230px;"><a href="consulta_turma_horario.php?ordem=5&id=<?php echo $_GET['id'];?>">Professor</a></th>
						</tr>
					</thead>
					<tbody>
					<?php 
						/* IMPRESSO DA LISTAGEM DE CURSOS */
						$query_grid = mysql_query("SELECT dia, horario, inicio, termino, disciplina.nome, professor.nome as pnome FROM horario, disciplina, professor, turma WHERE turma.cod_turma = ".$_GET['id']." AND disciplina.cod_professor = professor.cod_professor AND horario.cod_disciplina = disciplina.cod_disciplina AND turma.cod_turma = disciplina.cod_turma AND horario.cod_turma = turma.cod_turma AND horario.cod_professor = professor.cod_professor ORDER BY $ordem") or die ("Error na consulta");
