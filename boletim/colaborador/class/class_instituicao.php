...
class Class_instituicao
{
     ...
     public function insert_instituicao($nome,$email,$cod_cidade,$cod_bairro,$cod_rua,$complemento,$numero,$cod_uf,$telefone,$cod_status,$cep,$responsavel,$mensalidade, $contrato, $data_ass, $dia_base, $pagamento, $parcelas, $adesao, $pasta, $file, $tipo)
     {
         ...
         session_start();
		 $cod_colaborador = $_SESSION['id_usuario'];  
		...
		// se retornou então envia a mensagem
            if ($quant > 0)
            {
                ...
                $this->resultado = "email_usuario";
                 ...
                 {
                     $query_insert = mysql_query("INSERT INTO instituicao VALUE (NULL, '$nome','$email','$cod_cidade','$cod_bairro','$cod_rua','$complemento','$numero','$cod_uf','$telefone','$cod_status', '$cep', '$responsavel','$mensalidade', '$contrato', '$data_ass', '$dia_base', '$pagamento', '$parcelas', '$adesao','$pasta', '$file', '$tipo')") or die ("Erro insert inst". mysql_errno());
                     ...
                     $resultado = mysql_fetch_array($query_consulta);
                      ...
                      $senha =  md5($pasta."123");
                    $query_insert = mysql_query("INSERT INTO usuario VALUE (NULL, '$responsavel','$email','$senha',1,0,0,'$resultado[0]','$email')") or die ("Erro insert user". mysql_errno());
					
$query_insert = mysql_query("INSERT INTO config VALUE (7,2,30,'$resultado[0]',1,0)") or die ("Erro insert config". mysql_error());

$query_insert = mysql_query("INSERT INTO colaborador_instituicao VALUE (NULL,'$cod_colaborador','$resultado[0]')") or die ("Erro insert colab". mysql_error());
                    ...
                    fclose($fp);
                     ...
                     $query_grid = mysql_query("SELECT instituicao.cod_instituicao, instituicao.nome, cidade_nome , telefone FROM instituicao, cidade, colaborador_instituicao WHERE colaborador_instituicao.cod_instituicao = instituicao.cod_instituicao AND instituicao.cod_cidade = cidade_id  AND cod_colaborador = '$cod_colaborador' ORDER BY instituicao.nome",$conn) or die ("Error na consulta");
     ...
     public function select_grid_instituicao() { // método para preenchimento da grid no form
                    ...
                    $query_grid = mysql_query("SELECT instituicao.cod_instituicao, instituicao.nome, cidade_nome , telefone FROM instituicao, cidade, colaborador_instituicao WHERE colaborador_instituicao.cod_instituicao = instituicao.cod_instituicao AND instituicao.cod_cidade = cidade_id AND colaborador_instituicao.cod_colaborador = '$cod_colaborador' ORDER BY instituicao.nome",$conn) or die ("Error na consulta");
